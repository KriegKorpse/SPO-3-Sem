...
class Hash_values{
    ...
    int compare(Hash_values right) {
        ...
        for(int i = 0; i < 8; i++){
            if(right.h_v.get(i) > left.h_v.get(i)) {
                return -1;
            }
            else if(right.h_v.get(i) < left.h_v.get(i)){
                return 1;
            }
        }
        return 0;
...
class UInt {
    public static long cast_ulong(int a) {
        return a & 0x00000000FFFFFFFF;
    }

    public static int cast_uint(long a) {
        return (int) (a & 0x00000000FFFFFFFF);
    }
...
public class SHA256_Hash_Function {
    ...
    private static ArrayList<Byte> Int2ByteArr(int v) {
    ...
    private static int ByteConcat2Int(byte bt1, byte bt2, byte bt3, byte bt4) {
        int res = bt1;
    ...
    private static ArrayList<String> arr_54s(String txt) {
    ...
    public static Hash_values hash_func(String text) {
            ...
            for (int i = 0; i < text.length(); i++) {
                ...
                for (int j = 2; j < alphabet.length; j++) {
                    if (alphabet[j] == text.charAt(i)) {
                        arr.add(Integer.valueOf(j).byteValue());
                    }
                }
            }
